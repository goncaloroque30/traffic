...
from typing import AbstractSet, ClassVar, Dict, Iterable, Tuple
...
class Anp(object):
    ...
    def __init__(
        ...
        anp_substitution: str | None = None,
    ):
        """
        ...
        if anp_version in self.anp_url_map:
            ...
            self._anp_from_zip(Path(anp_version))

        if anp_substitution is None:
            self._substitution_download()
        else:
            self._substitution_from_xlsx(Path(anp_substitution))
    ...
    def takeoff_weight(
        ...
        stage_length: None | int = None,
        flight_distance: None | float = None,
    ) -> float:
        """
    ...
    # -- Table Filtering --
    def filter(self, by: str | Iterable[str] = "all") -> "Anp":
        """
        ...
        if "flap retraction" in by:
            r._filter(set(r.flap_retraction_schedule["ACFT_ID"]))
    ...
    def _filter(self, acft_ids: AbstractSet[str]) -> None:
        ...
        f = self.flap_retraction_schedule
    ...
    def thrust_rating(
        ...
        temp: array | None,
        press: array | None,
        cutback: float | int | None = None,
        vert_rate: array | None = None,
        ...
        if acft_id in self.jet_engine_coefficients["ACFT_ID"].values:  # type: ignore
            ...
            if not high_temperature:
                ...
                return np.concatenate([takeoff_thrust, climb_thrust]), int(idx)
    ...
    def _r_coefficient(self, acft_id: str, cas: array) -> array:
        acft_schedule = self.flap_retraction_schedule[
            self.flap_retraction_schedule["ACFT_ID"] == acft_id
        ]
        idx = np.maximum(
            np.searchsorted(
                -acft_schedule["Start CAS (kt)"].values,  # type: ignore
                -cas,
                side="right",
            )
            - 1,
    ...
    def _anp_download(self, version: str) -> None:
        f = client.get(self.anp_url_map[version])
        buffer = BytesIO()
        [buffer.write(chunk) for chunk in f.iter_bytes(chunk_size=1024)]

        file = self.cache_dir / "anp.zip"
        with open(file, "wb") as f_d:
            f_d.write(buffer.getbuffer())
    ...
    def _substitution_download(self) -> None:
        f = client.get(self.substitution_url)
        buffer = BytesIO()
        [buffer.write(chunk) for chunk in f.iter_bytes(chunk_size=1024)]
